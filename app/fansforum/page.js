...
import { MessageSquare, ArrowLeft, Search, Loader2, TrendingUp, Clock, User, MessageCircle } from 'lucide-react';
            ...
            {recentPosts.length === 0 ? (
              ...
              <div className="space-y-4">
                {recentPosts.map(post => (
                  <div
                    key={post.id}
                    onClick={() => router.push(`/fansforum/${post.club_id}/${post.id}?name=${encodeURIComponent(post.club_name)}&logo=${encodeURIComponent(getClubLogo(post.club_id))}`)}
                    className="bg-slate-800 rounded-xl p-6 border-2 border-slate-700 hover:border-orange-500 transition cursor-pointer"
                  >
                    <div className="flex items-start gap-4">
                      {/* Club Logo */}
                      <div className="flex-shrink-0">
                        {getClubLogo(post.club_id) ? (
                          <img 
                            src={getClubLogo(post.club_id)} 
                            alt={post.club_name}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-orange-400 text-sm font-medium">{post.club_name}</span>
                        </div>
                        <h3 className="text-xl font-bold text-white mb-2 truncate">{post.title}</h3>
                        <p className="text-slate-400 line-clamp-2 mb-3">{post.content}</p>
        ...
        {activeTab === 'clubs' && (
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              ...
              <div className="bg-slate-800 rounded-lg p-4 border-2 border-slate-700 text-center">
                <p className="text-3xl font-bold text-green-400">{recentPosts.length}</p>
